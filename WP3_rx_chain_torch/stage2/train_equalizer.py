...
def main():
    ...
    ckpt_path = os.path.join(ckpt_dir, 'stage1_final_checkpoint.pth')
    if os.path.exists(ckpt_path):
        receiver.load_state_dict(torch.load(ckpt_path, map_location=device))

    for p in receiver.channel_estimator.parameters():
        p.requires_grad = False
    
    params = list(receiver.equalizer.parameters()) + list(receiver.data_processor.parameters())
    ...
    for epoch in range(num_epochs):
        ...
        for step in range(steps_per_epoch):
            ...
            logits, _, _ = receiver.process(Y_t, Xp_t, pilot_pos_t, Nfft, 4, tx_bits_t)
            
            loss = bce(logits, tx_bits_t)
            loss.backward()
            optimizer.step()
        ...
        torch.save(receiver.state_dict(), os.path.join(ckpt_dir, f'stage2_equalizer_epoch{epoch+1}.pt'))
        
        final_epoch_loss = epoch_loss / steps_per_epoch
        scheduler.step(final_epoch_loss)
        
        if epoch == num_epochs - 1:
            torch.save(receiver.state_dict(), os.path.join(ckpt_dir, 'stage2_final_checkpoint.pth'))
