...
sys.path.append(os.path.join(os.path.dirname(__file__), '..', 'WP1_data_generator', 'WP1_data_generator_python'))

from functions import qammod, awgn
from rx_chain import rx_chain

# Load data
python_output_dir = os.path.join(os.path.dirname(__file__), '..', 'python_output')
h_perfect = np.load(os.path.join(python_output_dir,'h_perfect.npy'))
h_ls_estimation = np.load(os.path.join(python_output_dir,'h_ls_estimation.npy'))
...
for SNR in SNRs:
    ...
    plt.title(f'Channel Magnitude Response (M=1, SNR={SNR} dB)')
    plt.legend()
    plt.grid(True)
    ...
    plt.title(f'Channel Phase Response (M=1, SNR={SNR} dB)')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f'channel_estimation_M1_SNR{SNR}.png')
...
plt.title('BER vs SNR (M=1)')
plt.savefig('ber_vs_snr_M1.png')
plt.show()
